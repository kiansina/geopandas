...
df = pd.read_excel(r"C:\Users\sina.kian\Desktop\Sicit_graphics\Prodotto_Rev1 - Sina.xlsx",sheet_name=None)
...
for lbl in leg.get_texts()[:-1]:
    ...
    new_text = locale.format_string('%10.0f', float(re.sub(',', '',lower)) , grouping=True)+' - ' +locale.format_string('%10.0f', float(upper) , grouping=True)
...
ax.set_axis_off()
ax.set_title('Fatturato [€] di ogni regione del mondo 2020\nper il prodotto "Biostimolanti per agricoltura"')
leg = ax.get_legend()
for lbl in leg.get_texts()[:-1]:
    label_text = lbl.get_text()
    lower = label_text.split()[0]
...
WW=WW[['continent', 'name', 'iso_a3', 'gdp_md_est', 'geometry']]
ind=[]
df=pd.read_excel(r"C:\Users\sina.kian\Desktop\Sicit_graphics\P2py.xlsx")
WW['scope']=['No Value']*len(WW)
WW['value']=[0]*len(WW)
ll=[]
LL=[]
...
plt.gca().axes.xaxis.set_visible(False)
ax.set_axis_off()
ax.set_title('Fatturato [€] di ogni regione del mondo 2020\nper il prodotto "Grasso per biocombustibili"')
leg = ax.get_legend()
for lbl in leg.get_texts()[:-1]:
    label_text = lbl.get_text()
    lower = label_text.split()[0]
...
WW=WW[['continent', 'name', 'iso_a3', 'gdp_md_est', 'geometry']]
ind=[]
df=pd.read_excel(r"C:\Users\sina.kian\Desktop\Sicit_graphics\P3py.xlsx")
WW['scope']=['No Value']*len(WW)
WW['value']=[0]*len(WW)
ll=[]
LL=[]
...
plt.gca().axes.xaxis.set_visible(False)
ax.set_axis_off()
ax.set_title('Fatturato [€] di ogni regione del mondo 2020\nper il prodotto "Ritardante per gesso"')
leg = ax.get_legend()
for lbl in leg.get_texts()[:-1]:
    label_text = lbl.get_text()
    lower = label_text.split()[0]
...
WW=WW[['continent', 'name', 'iso_a3', 'gdp_md_est', 'geometry']]
ind=[]
df=pd.read_excel(r"C:\Users\sina.kian\Desktop\Sicit_graphics\P4py.xlsx")
WW['scope']=['No Value']*len(WW)
WW['value']=[0]*len(WW)
ll=[]
LL=[]
...
WW=WW[['continent', 'name', 'iso_a3', 'gdp_md_est', 'geometry']]
ind=[]
df=pd.read_excel(r"C:\Users\sina.kian\Desktop\Sicit_graphics\Total.xlsx")
WW['scope']=['No Value']*len(WW)
WW['value']=[0]*len(WW)
ll=[]
LL=[]
